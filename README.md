...
# short explain
ProteaseSimNmer estimates the probability with which each protease cleaves specific amino acid sites within the spike proteins of representative SARS-CoV-2 variants.

# explain
Using a training dataset generated by ProteaseDatasetGenerator—where positive data consists of N-mer segments spanning protease cleavage sites and negative data consists of randomly selected N-mer segments from non-cleavage regions—we estimate the cleavage probability for spike proteins of representative SARS-CoV-2 variants by sliding an N-mer window frame-by-frame from the N-terminus to the C-terminus.
